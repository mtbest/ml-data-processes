...
merged_data <- d_pre %>% full_join(d_post, by = c("PartID"), suffix = c("_pre", "_post"))
...
merged_data <- d[c(22:60),]

# Selecting certain columns by index

merged_data <- d[,c(8:27)]
...
newdata4 <- merged_data %>% 
  filter(D_female_pre == "female" & D_race_ML_pre == "Black or African American")
newdata5 <- merged_data %>%
  filter(D_female_pre == "male" & D_race_ML_pre == "Black or African American")
...
merged_data <- merged_data %>% 
  mutate(St1_S_gen_pre = ((St1_bel1_pre + St1_bel2_pre) / 2), 
         St1_P_gen_pre = ((St1_v1_pre + St1_v2_pre) / 2), 
         St1_INTER_m_pre = ((St1_mot_a_5_pre + St1_mot_a_6_pre + St1_mot_a_7_pre + St1_mot_a_8_pre) / 4))
...
merged_data <- merged_data %>% 
  mutate(St1_S_gen_pre = rowMeans(c("St1_bel1_pre","St1_bel2_pre"), na.rm = TRUE)) %>%
  mutate(St1_P_gen_pre = rowMeans(c("St1_v1_pre","St1_v2_pre"), na.rm = TRUE)) %>%
  mutate(St1_INTER_m_pre = rowMeans(c("St1_mot_a_5_pre", "St1_mot_a_6_pre", "St1_mot_a_7_pre", "St1_mot_a_8_pre"), na.rm = TRUE))
...
merged_data %>%
  mutate(St1_P_gen_pre = rowMeans(select(., starts_with("St1_v")), na.rm = TRUE)
...
table <- merged_data[c("St1_S_gen_pre", "St1_P_gen_pre", "St1_INTER_m_pre")]

table %>% tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), 
                       type = list(St1_S_gen_pre ~ 'continuous',
                                   St1_P_gen_pre ~ 'continuous', 
                                   St1_INTER_m_pre ~ 'continuous'),
...
b1<- summarySE(merged_data, measurevar="St1_INTER_m_pre", na.rm=TRUE)
b2<- summarySE(merged_data, measurevar="St1_mot_a_5_pre", na.rm=TRUE)
b3<- summarySE(merged_data, measurevar="St1_mot_a_6_pre", na.rm=TRUE)
b4<- summarySE(merged_data, measurevar="St1_mot_a_7_pre", na.rm=TRUE)
b5<- summarySE(merged_data, measurevar="St1_mot_a_8_pre", na.rm=TRUE)
...
b0 <- map(
  .x = c("St1_mot_a_5_pre", "St1_mot_a_6_pre", "St1_mot_a_7_pre", "St1_mot_a_8_pre"),
...
merged_data %>% 
  select(St1_mot_a_5_pre, St1_mot_a_6_pre, St1_mot_a_7_pre, St1_mot_a_8_pre, St1_INTER_m_pre) %>%
  map(psych::describe)
...
nd_data <- merged_data[!duplicated(merged_data$PartID_pre),]
...
merged_data <- merged_data %>%
    mutate_at(c("St1_toi1_pre", "St1_toi2_pre"),
              ~ recode(., "1" = 6, "2" = 5, "3" = 4, 
                          "4" = 3, "5" = 2, "6" = 1))
